...
%%
N=600;
interpolant = scatteredInterpolant(x,y,nu,'linear','linear');
xx = linspace(-R,R,N);
yy = linspace(-R,R,N);
[xx,yy] = meshgrid(xx,yy);
nu = interpolant(xx,yy);



nu = nu/nu_w;
 nu_radial = nu(N/2,1:N/2);
 yPlus = (R+ xx(N/2,1:N/2))/lengthScale;
 plot(yPlus, nu_radial); hold on;
 set(gca, 'XScale', 'log')

% Create a table from your data
T = table(yPlus(:), nu_radial(:), 'VariableNames', {'yPlus','uv_radial'});

% Write the table to a CSV file named "myData.csv"
writetable(T, 'viscosity_HB10_SSTHB.csv');
%% Save figure
...
% data1 = readtable('SinghData.csv');
% data1=data1{:,:};
% time1=data1(:,1);
% vel1= data1 (:,2);
% 
%  plot(time1, vel1); hold on;

% print -depsc2 -painters 'Pe=5e2_t=15.eps'
% saveas(gca, figureFileName, 'png');
