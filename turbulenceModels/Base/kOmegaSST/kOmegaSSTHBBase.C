...
void kOmegaSSTHBBase<TurbulenceModel, BasicTurbulenceModel>::correct()
{
    ...
    volScalarField extraNN = C_beta*betaStar_*omega_*k_;  //extra term for non-Newtonian
    ...
    volScalarField nu_modified = tau0 / gammaDot + K * pow(gammaDot, n - 1.0); // initilization
...

    {
        ...
        (
          ...
          - fvm::laplacian(alpha*rho*nu_modified, omega_) // Modified equation
         ==
            alpha()*rho()*gamma
           *min
            (
    ...
    (
      ...
      - fvm::laplacian(alpha*rho*nu_modified, k_)  //Modified equation
     ==
        alpha()*rho()*Pk(G)
